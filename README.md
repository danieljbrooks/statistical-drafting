...
GitHub Actions automatically checks for new data nightly. When updates are detected, train new models with `cd model_refresh && python refresh_models.py` (PyTorch `.pt` plus ONNX export by default), then manually deploy ONNX models to the website repository.
